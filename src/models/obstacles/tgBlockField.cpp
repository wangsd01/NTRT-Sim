...
void tgBlockField::addNodes(tgStructure& s) 
{
    ...
    for(size_t i = 0; i < 2 * m_config.m_nBlocks; i += 2) 
    {
        ...
        do
        {
            x = m_config.m_minPos.getX()+fieldSize.getX() * rand() / RAND_MAX;
            y = m_config.m_minPos.getY()+fieldSize.getY() * rand() / RAND_MAX;
            z = m_config.m_minPos.getZ()+fieldSize.getZ() * rand() / RAND_MAX;
        }
