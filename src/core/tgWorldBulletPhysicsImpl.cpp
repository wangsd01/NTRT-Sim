...
void tgWorldBulletPhysicsImpl::step(double dt)
{
    ...
    // m_pIntermediateBuildProducts->broadphase.resetPool(&m_pIntermediateBuildProducts->dispatcher);
    // m_pIntermediateBuildProducts->solver.reset();

    const btScalar timeStep = dt;
    const int maxSubSteps = 0;
