...
void tgBulletSpringCable::calculateAndApplyForce(double dt)
{
    ...
    // if(PRINT_FLAG)
    // {
    //     if(fabs(currLength - m_prevLength)<.000000000000001)
    //     {
    //         std::cout<<"Value matches!!!!"<<std::endl;
    //     }
    //     else
    //     {
    //         std::cout<<std::endl<<std::fixed << std::setprecision(15)<<"Actual: "<<currLength<<" Stored: "<<m_prevLength<<std::endl;
    //     }
    // }
...
void tgBulletSpringCable::setPreviousLength( const double newPreviousLength)
{
    // Assume we've already put this through a motor model
    // But check anyway
    assert(newPreviousLength >= 0.0);
    int n = m_anchors.size();
    m_prevLength = newPreviousLength;
    // btVector3 pos1 = m_anchors[0]->getWorldPosition();
    // btVector3 pos2 = m_anchors[n - 1]->getWorldPosition();
    // m_prevLength = pos1.distance(pos2);
    // m_restLength = pos1.distance(pos2) - m_pretension / m_coefK;
}
