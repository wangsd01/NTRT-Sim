...
void tgBulletContactSpringCable::updateManifolds()
{
    ...
    // std::cout<<numPairs<<" "<<m_anchors.size()<<std::endl;
    // std::cout.flush();
	...
	for (int i = 0; i < numPairs; i++)
	{
	    ...
	    // std::cout<<"---- "<<collisionPair<<std::endl;
	    // std::cout.flush();
					...
					if (dist < 0.0)
					{
						...
						{  
							...
							if (anchorPos >= 0)
							{
								...
								btPersistentManifold* new_manifold = new btPersistentManifold();
								*new_manifold = *manifold;
								tgBulletSpringCableAnchor* const newAnchor = new tgBulletSpringCableAnchor(rb, pos, m_touchingNormal, false, true, new_manifold);
		...
		}//if this pair has something to process
	} // For pairs of objects

	
}

tgBulletSpringCableAnchor* tgBulletContactSpringCable::addAnchor(btRigidBody* rb, btVector3 pos, btVector3 m_touchingNormal, bool permanent, bool sliding, btPersistentManifold* new_manifold, int index_to_insert)
{
	tgBulletSpringCableAnchor* newAnchor = new tgBulletSpringCableAnchor(rb, pos, m_touchingNormal, permanent, sliding, new_manifold);
	newAnchor->setRelativeOriginalPosition(pos);

	m_anchorIt = m_anchors.begin() + index_to_insert;
...
void tgBulletContactSpringCable::stateUpdateCollisionObject()
{
#ifndef BT_NO_PROFILE 
