...
{ 
	double maxTorque = m_config.maxTens * m_config.radius * 
						(1.0 - m_config.radius * fabs(m_motorVel) / m_config.targetVelocity);
	
	maxTorque = maxTorque < 0.0 ? 0.0 : maxTorque;
	
	double ret = fabs(desiredTorque) <= maxTorque ? desiredTorque : 
		desiredTorque / fabs(desiredTorque) * maxTorque;
    // if(ret!=ret)
    // {
    //     std::cout<<desiredTorque<<" "<<maxTorque<<std::endl;
    //     std::cout.flush();
    // }
    return ret;
}
